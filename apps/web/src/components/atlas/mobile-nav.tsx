...
export function AtlasMobileNav({ activePath }: { activePath: string }) {
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-50 flex border-t border-border bg-background/95 px-1 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl lg:hidden"
      aria-label="Mobile"
    >
      {ATLAS_NAV.map((item) => {
      ...
      <Link
        ...
        className={cn(
          "flex flex-1 flex-col items-center gap-1 py-2.5 text-[10px] font-semibold transition-colors",
          isNavActive(activePath, ATLAS_SETTINGS.href) ? "text-primary" : "text-muted-foreground",
        )}
        aria-label="Settings"
      >
        <span
          className={cn(
            ...
            isNavActive(activePath, ATLAS_SETTINGS.href) && "bg-primary/12",
          )}
        >
